...
<%@ include file="addBiddingOne.jsp"%>
<body>
...
//TODO:	招标内容开始
	/* =============================================招标内容  start================================================ */
		...
		function quantityRenderer(val){
      		...
      		return "<input name='biddingContentItem[][quantity]' type='text' class='biddingDetailContentColsEdit quantityClick' onkeyup='value=value.replace(/[^(0-9\\.)]/g,\"\")' onclick='value=clearData(this.value)' value='"+val+"'/>";
      	}
		function fbk2Renderer(val){
      		if(!val){
      			val="";
      		}
      		return "<input name='biddingContentItem[][fbk2]' type='text' class='biddingDetailContentColsEdit' onkeyup='value=value.replace(/[^(0-9\\.)]/g,\"\")' onclick='value=clearData(this.value)' value='"+val+"'/>";
      	}
		function fbk3Renderer(val){
      		if(!val){
      			val="";
      		}
      		return "<input name='biddingContentItem[][fbk3]' type='text' class='biddingDetailContentColsEdit' onkeyup='value=value.replace(/[^(0-9\\.)]/g,\"\")' onclick='value=clearData(this.value)' value='"+val+"'/>";
      	}
		...
		function fbk9Renderer(val){
			if(!val){val = "";}
      		return "<input name='biddingContentItem[][fbk9]' type='text' value='17%'/>";
      	}
...
/**----------============需求类型和编码选择（END）============----------*/
	...
	$('.edit-line-content').click(function() {
		...
		$("body").data("contentBiddingItemNumber_local",$("#contentBiddingItemNumber").val());
	});
	...
	$("#biddingStandardRemove").click(function(){
 	...
 	$("#contentBiddingItemNumber").change(function(){
		...
		if(typeof(biddingItemNumber) == "undefined"){
			//biddingItemNumber = form.biddingContentItem[0].biddingItemNumber;//原来代码
			biddingItemNumber = 1;//暂时测试代码
		}
		...
		if("undefined" !=typeof(biddingContentItemStandardArray[selectValue])){
			...
			var deliveryStartDate_ = setBiddingContent.deliveryStartDate;
			var deliveryEndDate_ = setBiddingContent.deliveryEndDate;
			if(deliveryStartDate_ != "" && typeof(deliveryStartDate_) != "undefined"){
				$("#contentDeliveryStartDate").val(new Date(deliveryStartDate_).format('yyyy-MM-dd'));
			}else{
				$("#contentDeliveryStartDate").val(new Date().format('yyyy-MM-dd'));
			}
			if(deliveryEndDate_ != "" && typeof(deliveryEndDate_) != "undefined"){
				$("#contentDeliveryEndDate").val(new Date(deliveryEndDate_).format('yyyy-MM-dd'));
			}
...
// 						parent.elsDeskTop.hideLoading();
	...
	var colsQueryMaterial = [
		         	    ...
		         	    { title:'<i18n:I18n key="" defaultValue="数量单位"/>', name:'purchaseUnit' , width:140, align:'center'}
		         	     ];	
	 ...
	 */
	 $("#btnQueryMaterialOK").click(function(){
		...
		for(var i = 0 ; i < selectRows.length ; i ++){
			...
			item['biddingItemNumber']=$("#contentBiddingItemNumber").val();
...
/** -----------===============Excel导入导出、评标导入、需求计划提取（结束）===============----------- */
	...
	var editFormTable;
	var fileUploader;
	var fileUploader_back;//回签表单文件对象
	...
	function initFileUploader2(fileUploader_back){
		   if(!fileUploader_back){
		    //新增表单文件
		  	fileUploader_back = WebUploader.create({
		  	    // swf文件路径
		  	    swf: '<%=basePath%>jsLib/webuploader/Uploader.swf',
		  	  	duplicate :true,
		  	    auto: true,
		  	...
		  	fileUploader_back.on( 'uploadAccept', function( obj, response ) {
		  	  	if(0 == response.error){
		  	   	   var filePath = response.url;
		  	   		$("#filePath1").val(filePath);
		...
		$("#uploadForm").click(function(){
			 initFileUploader2(fileUploader_back);
			 $('#addFileDialog1').show();
		});
