								...
								<tbody>
									...
									<tr>
										...
										<td colspan="1" align="left">
											...
											<input id="projectRemark" name="projectRemark" type="text" readonly="readonly" style="width: 300px;" />
										</td>
									</tr>
								</tbody>
							</table>
...
<!-- ----------------------------------------在线竞价 @author jiangzhidong @date 20160324  end------------------------ -->                    
				...
				<div id="edit-evalute-target">
					...
					<div class="box-wrap pos-relative bg-common">
						...
						<div id="sendTenderBidConfig" class="common-box-line">
							<input type="checkbox" id="chkSendWonBidQute" value="0">
								发送份额
							</input>
							&nbsp;&nbsp;
							<input type="checkbox" id="chkSendWonBidRank" value="0">
								发送排名
							</input>
						</div>
						<div class="common-box-line">
...
//     parent.elsDeskTop.closeCurWin('auditBiddingBargainManage');
// });
//审批begin
if (auditType == "todo") {
	$(".todo").show();
	...
	parent.elsDeskTop.tip.init({type: 'confirm',message:'<i18n:I18n key="i18n_common_confirm_auditpass" defaultValue="确认要审批通过吗？" />',closeEvent:function(){
		...
		if(biddingNumber == "null" || biddingNumber == ""){		
			...
			if(currentBiddingItemNumber != null){
				...
				formObj["biddingItemNumber"]=currentBiddingItemNumber;
			}else{
				formObj["biddingNumber"]=biddingNumber;
				formObj["elsAccount"]=elsAccount;
				formObj["elsSubAccount"]=elsSubAccount;
		...
		var param = formObj;
		$.ajax({
			url :"<%=basePath%>rest/PurchaseBiddingService/sendBiddingPass",
			type :"post",
			contentType : "application/json",
...
// 		}
		...
		$.ajax({
			url :"<%=basePath%>rest/PurchaseBiddingService/sendBiddingNoPass",
			type :"post",
			contentType : "application/json",
...
$("#itemColumnDefineBtn").click(function(){
	 parent.elsDeskTop.defineWin({
	     ...
	     'iframSrc':'<%=basePath%>tablecolumn/subAccountColumnSetting.jsp?elsAccount=' +
	    		 ...
	    		 "&tableCode=purchaseEnquiryBargainItem" +
...
function init() {
	debugger;
	if(biddingNumber.split("-")[1] != null){
		currentBiddingItemNumber = biddingNumber.split("-")[1];
		biddingNumber = biddingNumber.split("-")[0];
	}
			...
			*/
			...
			if(biddingMarginVOs &&  biddingMarginVOs.length>0) {
				...
				if(multiSection == 0) {
					...
					if(currentBiddingItemNumber == ""){
 						$("#margin_distinguishPhases").val("1");//默认选择标段1
						$("body").data("margin_distinguishPhases","1");
					}else{
						$("#margin_distinguishPhases").val(currentBiddingItemNumber);
						$("body").data("margin_distinguishPhases",currentBiddingItemNumber);
					}
					debugger;
...
// 						if(biddingItemNumber == "1"){//第一标段数据
						if(currentBiddingItemNumber != ""){
							if(biddingItemNumber == currentBiddingItemNumber){//第一标段数据
			...
			if(biddingContentVOs &&  biddingContentVOs.length>0) {
				...
				if(biddingPhasesItems && phaseItemsLen > 1) {
					 ...
					 for(var i=0;i<biddingContentVOs.length;i++) {
						 if(currentBiddingItemNumber != ""){
							 if(biddingContentVOs[i].biddingItemNumber==currentBiddingItemNumber) {
					 ...
					 for(var j=0;j<biddingContentItemVOs.length;j++) {
						 if(currentBiddingItemNumber != ""){
							 if(biddingContentItemVOs[j].biddingItemNumber==currentBiddingItemNumber) {
					 ...
					 for(var i=0;i<biddingContentStandardVOs.length;i++) {
						 if(currentBiddingItemNumber != ""){
							 if(biddingContentStandardVOs[i].biddingItemNumber==currentBiddingItemNumber) {
					 ...
					 if(currentBiddingItemNumber != ""){
						 $("#contentBiddingItemNumber").val(currentBiddingItemNumber);//默认选择标段1
						 $("body").data("contentBiddingItemNumber_local",currentBiddingItemNumber);
					 }else{
			...
			if(biddingContentVOs &&  biddingContentVOs.length>0) {
				...
				if(biddingPhasesItems && phaseItemsLen > 1) {
					 ...
					 for(var i=0;i<biddingPhasesItems.length;i++) {
						...
						// 默认第一标段的轮次
						 if(currentBiddingItemNumber != ""){
							 if(biddingPhasesItems[i].biddingItemNumber==currentBiddingItemNumber) {
					 ...
					 if(currentBiddingItemNumber != ""){
					 	 setRoundNumberObj2.val(currentBiddingItemNumber);
						 $("body").data("contentBiddingRoundNumber_local2",currentBiddingItemNumber-1);
						 setItemNumberObj2.val(currentBiddingItemNumber);
						 $("body").data("contentBiddingItemNumber_local2",currentBiddingItemNumber-1);
						 setBiddingContent2=biddingContentVOs[currentBiddingItemNumber-1];
						 $("#contentBiddingItemNumber2").val(currentBiddingItemNumber);
					 }else{
					 // 设置当前默认标段轮次
				...
				setBiddingContent2 = biddingContentVOs[0];
				//$("#descriptionArea").val(setBiddingContent2.summary);
