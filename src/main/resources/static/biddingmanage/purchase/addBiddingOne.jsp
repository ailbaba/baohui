...
<%@ include file="addBiddingMain.jsp"%>
<body>
...
function computeOperation(){
	 ...
	 if(biddingType == "2" || biddingType == "3"){ //在线竞标
		 for(var i = 0 ; i < biddingRounds ; i ++){
			 //招标类型 @author update/jiangzhidong @date 20161219 begin 
			 if(!endDate){
				 beginDate = new Date(biddingBeginDate+" "+biddingBeginTime);
				 endDate=new Date();
				 endDate.setTime(beginDate.getTime()+1000*60*eachRound);
				 var item = {"biddingType":"2","biddingItemNumber":phasesNumber,"roundsNumber":i+1,"biddingStatus":"0","biddingBeginDate":biddingBeginDate,"biddingBeginTime":biddingBeginTime,"biddingEndDate":endDate.format('yyyy-MM-dd'),"biddingEndTime":endDate.format('hh:mm:ss'),"initPrice":budgetAmount,"priceOff":"","unitsType":""};
				 phasesRoundTable.addRow(item);
			 }else{
				 beginDate.setTime(endDate.getTime()+1000*60*roundInterval);
				 endDate.setTime(beginDate.getTime()+1000*60*eachRound);
				 var item = {"biddingType":"2","biddingItemNumber":phasesNumber,"roundsNumber":i+1,"biddingStatus":"0","biddingBeginDate":beginDate.format('yyyy-MM-dd'),"biddingBeginTime":beginDate.format('hh:mm:ss'),"biddingEndDate":endDate.format('yyyy-MM-dd'),"biddingEndTime":endDate.format('hh:mm:ss'),"initPrice":"0","priceOff":"","unitsType":""};
				 phasesRoundTable.addRow(item);
			 }
			//招标类型 @author update/jiangzhidong @date 20161219 end
		 }
	 }else{//普通招投标
...
$("#plan_distinguishPhases").change(function(){
	...
	$("#phases-round-table :checkbox").each(function(){
		$(this).attr("disabled",false);
	});
	...
	if(typeof(roundItem) != "undefined" && roundItem != null){
		for(var i=0 ; i<roundItem.length ; i++){
			...
			formObj["biddingRoundItms"][i]["fbk5"] = roundItem[i].fbk5;
			formObj["biddingRoundItms"][i]["fbk6"] = roundItem[i].fbk6;
			formObj["biddingRoundItms"][i]["fbk7"] = roundItem[i].fbk7;
			formObj["biddingRoundItms"][i]["fbk8"] = roundItem[i].fbk8;
		}
	}
	debugger;
	...
	if(!roundItms || roundItms.length == 0 || !roundItms[0]){
		alert("招标计划-表格不能为空！请输入必填项请输入必填项",2);
	...
	if(biddingStatus != 0 && biddingStatus != 3 && biddingStatus != "null"){
		$("#phases-round-table :text").each(function(){
		...
		var _fbk8 = roundItems[roundsNum-1].fbk8;//是否人工干预
...
$("#margin_distinguishPhases").change(function(){
	...
	if(!biddingMargins){//不存在保证金信息
		...
		$('input[name="biddingMarginVO[collect]"]:eq(0)').attr("checked",true);
		...
		for(var i = 0 ; i < biddingMargins.length ; i ++){
			...
			if(_biddingItemNumber == currentBiddingItemNumber){
				...
				if(item.payBeginDate != "" && item.payBeginDate != null){
					$("#margin_payBeginDate").val(new Date(item.payBeginDate).format('yyyy-MM-dd'));
				}
				if(item.payEndDate != "" && item.payEndDate != null){
...
$("#send").click(function(){
	...
	/* if($("#biddingType").val() == "0") { 
		
	} else if($("#biddingType").val() == "2") {
		sendBidding();
	} */
});
//保存招投标信息-----end--------------
...
function operation(url,message1,message2,operType){
	...
	// 未审批不能发布拦截
	/* 
	if(preAuditStatus != "0" && operType == "2"){
		alert("未审批通过不能发布招标！",2);
		return;
	}
	 */
	...
	if(biddingNumber != ""){//判断基本信息已经保存
		...
		if(multiSection == "0"){//不区分 标段
			...
			if(biddingRounds){
				...
				//日期格式化
				for(var i = 0 ; i < biddingRounds.length ;i++){
					// 代码容错-处理日期  @author jiangzhidong @date 20161213 begin
					var beginDate = biddingRounds[i].beginDate;
					if(!isNaN(beginDate.replace(/-/g,''))){
						biddingRounds[i]["beginDate"] = new Date(beginDate).format('yyyy-MM-dd');
					}else{
						biddingRounds[i]["beginDate"] = new Date().format('yyyy-MM-dd');
					}
					var biddingBeginDate = biddingRounds[i].biddingBeginDate;
					if(!isNaN(biddingBeginDate.replace(/-/g,''))){
						biddingRounds[i]["biddingBeginDate"] = new Date(biddingBeginDate).format('yyyy-MM-dd');
					}else{
						biddingRounds[i]["biddingBeginDate"] = new Date().format('yyyy-MM-dd');
					}
					var biddingEndDate = biddingRounds[i].biddingEndDate;
					if(!isNaN(biddingEndDate.replace(/-/g,''))){
						biddingRounds[i]["biddingEndDate"] = new Date(biddingEndDate).format('yyyy-MM-dd');
					}else{
						biddingRounds[i]["biddingEndDate"] = new Date().format('yyyy-MM-dd');
					}
					var openDate = biddingRounds[i].openDate;
					try {
						if((openDate!='undefined')&&!(isNaN(openDate.replace(/-/g,'')))){
							biddingRounds[i]["openDate"] = new Date(openDate).format('yyyy-MM-dd');
						}else{
							biddingRounds[i]["openDate"] = new Date().format('yyyy-MM-dd');
						}
					} catch (e) {
						biddingRounds[i]["openDate"] = new Date().format('yyyy-MM-dd');
						console.log(e.message);
					}
					// 代码容错-处理日期  @author jiangzhidong @date 20161213 end
				}
		...
		if(typeof(multiSection) == "undefined"){//没有点保证金选项卡情况
			var biddingPhasesItemsArray = formObj.biddingPhasesItems;
			if(typeof(biddingPhasesItemsArray) == "undefined"){//不区分标段
				biddingMarginVO["multiSection"] = "0";
				biddingMarginVOs.push(biddingMarginVO);
				formObj["biddingMarginVOs"]=biddingMarginVOs;
			}else{//区分标段
				for(var i=0;i<biddingPhasesItemsArray.length;i++){
					biddingMarginVOs[i] = {};
					biddingMarginVOs[i]["biddingItemNumber"] = biddingPhasesItemsArray[i].biddingItemNumber;
					biddingMarginVOs[i]["multiSection"] = "1";
					biddingMarginVOs[i]["back"] = back;
					biddingMarginVOs[i]["collect"] = collect;
					biddingMarginVOs[i]["amount"] = amount.replace(/,/g,"");
				}
				formObj["biddingMarginVOs"]=biddingMarginVOs;
			}			
		}else if(multiSection == "0"){//不区分标段
			biddingMarginVOs.push(biddingMarginVO);
			formObj["biddingMarginVOs"]=biddingMarginVOs;
		}else{
			...
			if(biddingMargins){
				...
				//日期格式化
				for(var i = 0 ; i < biddingMargins.length ;i++){
					var payBeginDate_ =  biddingMargins[i].payBeginDate;
					var payEndDate_ = biddingMargins[i].payEndDate;
					if(payBeginDate_ != ""){
		...
		if(biddingContentVOsTemp && biddingContentVOsTemp != "" && biddingContentVOsTemp.length == itemPageNumber){
			for(var i=0;i<biddingContentVOsTemp.length;i++){//循环标段
				...
				if(deliveryStartDate_Str == "" || deliveryEndDate_Str == ""){
					alert("招标内容-生效日期不为空！",2);
					return;
				}
				if(deliveryStartDate < toDate(currDate)){
					alert("招标内容-生效日期设置有误！",2);
					return;
				}
				
				if(!biddingContentItemTemp || biddingContentItemTemp == ""){
					alert("招标内容-招标详细内容表格不能为空！",2);
					return;
				}else{
					for(var k=0;k<biddingContentItemTemp.length;k++){
						var quantity = biddingContentItemTemp[k].quantity;
						var fbk2 = biddingContentItemTemp[k].fbk2;
						var fbk3 = biddingContentItemTemp[k].fbk3;
						if(!quantity || quantity=="" || quantity=="0"){
							alert("招标内容-招标详细内容-物料数量不能为零或者空！",2);
							return;
						}
						if(!fbk2 || fbk2=="" || fbk2=="0"){
							alert("招标内容-招标详细内容-目标价不能为零或者空！",2);
							return;
						}
						if(!fbk3 || fbk3=="" || fbk3=="0"){
							alert("招标内容-招标详细内容-参考价不能为零或者空！",2);
							return;
						}
					}
				}
				if(!biddingContentStandardTemp || biddingContentStandardTemp.length == ""){
					//alert("招标内容-评标标准内容表格不能为空！",2);
					//return;
				} else {
					if(operType==2) {
						// 判断权重总和必须为100
			...
			alert("招标内容-招标详细内容表格不能为空！请输入必填项",2);
			return;
		}
	...
	parent.elsDeskTop.tip.init({type: 'confirm',message: message1,closeEvent:function(){
		...
		if($("#biddingTypeMudalHid").val()==7)
			formObj["biddingType"]=7;
		$.ajax({
			url :url,
			type :"POST",
			...
			success : function(data) {
				if($("#biddingTypeMudalHid").val()==7) {
					...
					window.location.href = '<%=basePath%>biddingmanage/purchase/addBiddingThree.jsp'
												+ '?biddingNumber='+data.biddingNumber
												+ '&biddingStatus=0&type=add';
				} else {
					parent.elsDeskTop.hideLoading();
					var biddingNumber = data.biddingNumber;
					$("#biddingNumber").val(biddingNumber);
					$("#biddingNumberView").val(biddingNumber);
					$(".tag-isShow").show();
					$("#biddingPhases").attr("disabled",true);
					$("#biddingType").attr("disabled",true);
					var biddingStatus = data.biddingStatus;
					if(biddingStatus == 4){//已发布
						$(".publish-isShow").hide();
						$(".save-isShow").hide();
					}else{
						$(".publish-isShow").show();
...
//     });
// }
$("#cancelPreBiddingBtn").click(function(){
		cancel();
	});
function cancel() {
	parent.elsDeskTop.tip.init({type: 'confirm',message:'<i18n:I18n key="i18n_common_confirm_auditcancel" defaultValue="确认要撤销审批吗？" />',closeEvent:function(){
		...
		$.ajax({
			url :"<%=basePath%>rest/PurchaseBiddingService/cancelAuditPreBidding",
			type :"post",
			contentType : "application/json",
			data:JSON.stringify(formObj),
			dataType : "json",
...
$("#entryPreBiddingBtn").click(function(){
	if($("#biddingNumberView").val()=='') {
		alert('基本信息-招标编号不能为空!请保存',2);
		return;
	}
	var formObj = $("#form").serializeJSON();
	var currDate = new Date().format("yyyy-MM-dd");
	...
	if(biddingNumber != ""){//判断基本信息已经保存
			...
			for(var a=0;a<roundsArrays.length;a++){//循环标段	
				...
				for(var i=0;i<biddingRounds.length;i++){//循环标段下的轮次行记录
					...
					if(biddingType == "0"){
						...
						if(beginDT < toDateAndTime(currDate,currTime)){
							alert("招标计划-投标开始日期有误",2);
							return;
						}
						
						if(i==0 && beginDateTime > endDateTime || endDateTime > openDateTime){//第一轮
							alert("招标计划-投标开始时间有误",2);
							return;
						}else if(i!=0){//非第一轮要比较当前轮的招标开始时间和上一轮的开标时间
							var prevRoundOpenDateTime = toDateAndTime(biddingRounds[i-1].openDate,biddingRounds[i-1].openTime);
							if(prevRoundOpenDateTime > beginDateTime || beginDateTime > endDateTime || endDateTime > openDateTime){
			...
			for(var i=0;i<biddingContentVOsTemp.length;i++){//循环标段
				...
				if(!biddingContentItemTemp || biddingContentItemTemp == ""){
					...
					for(var k=0;k<biddingContentItemTemp.length;k++){
						...
						var fbk3 = biddingContentItemTemp[k].fbk3;
						if(!quantity || quantity=="" || quantity=="0"){
							alert("招标内容-物料数量不能为零或者空！",2);
							return;
						}
						if(!fbk2 || fbk2=="" || fbk2=="0"){
							alert("招标内容-目标价不能为零或者空！",2);
							return;
						}
						if(!fbk3 || fbk3=="" || fbk3=="0"){
							alert("招标内容-参考价不能为零或者空！",2);
							return;
						}
					}
				}
		...
		formObj["biddingApplicationVOs"]=getBiddingApplicationVOs();
		debugger;
		var preAuditStatus = $("#preAuditStatus").val();
		if(preAuditStatus == "2"){
			var errMsg = "审批中的招标"+biddingNumber+"不能再次提交";
			alert(errMsg,2);
			return;
		}
		if(preAuditStatus == "0"){
			var errMsg = "审批通过的招标"+biddingNumber+"不能再次提交";
			alert(errMsg,2);
			return;
		}
		var stretegyCode=$("#stretegySelctor").val();
		debugger;
		formObj["stretegyCode"] = stretegyCode;
		var data = {"stretegyCode":stretegyCode,"elsAccount":elsAccount,"elsSubAccount":elsSubAccount,"companyShortName":companyShortName,"biddingNumber":biddingNumber};
		//获取当前选中的
		parent.elsDeskTop.tip.init({type: 'confirm',message:'<i18n:I18n key="i18n_common_are_you_sure_you_want_to_do_this" defaultValue="您确定要提交审批吗？" />',closeEvent:function(){
			parent.elsDeskTop.showLoading();
	    	$.ajax({
...
$("#supplierSend").click(function(){
	...
	//新增供应商邀请招标消息参数添加  @songZhen 20161230 begin
	if($("#elsSubAccount").attr("disabled")=="disabled"){
		$("#elsSubAccount").attr("disabled",false);
	}
	if($("#biddingName").attr("disabled")=="disabled"){
		$("#biddingName").attr("disabled",false);
	}
	$('input[name="fbk6"]').each(function(){
		if($(this).attr("disabled")=="disabled"){
			$(this).attr("disabled",false);
		}
	});
	//新增供应商邀请招标消息参数添加  @songZhen 20161230 end
	var formObj = $("#form").serializeJSON();
	var fbk6 = $('input[name="fbk6"]:checked').val(); //目标价是否发给投标方
	formObj["fbk6"]=fbk6;
	var biddingSupplierVOs = getBiddingSupplierVOs();
...
$("#roundsStatusUpdate").click(function(){
	$("#biddingNumber").attr("disabled",false);
	$("#elsAccount").attr("disabled",false);
	var formObj = $("#form").serializeJSON();
	var biddingRoundsVO = formObj.biddingRoundsVO;
	...
	biddingRoundsVO["biddingItemNumber"] = biddingItemNumber;
	var multiSection = $('input[name="biddingRoundsVO[multiSection]"]:checked').val();//是否区分标段
	var roundItems = formObj.biddingRoundItms;//获取轮次列表
	var biddingRoundsVOs = [];
	...
	$("#biddingNumber").attr("disabled",true);
	$("#elsAccount").attr("disabled",true);
	
	parent.elsDeskTop.tip.init({type: 'confirm',message: "轮次状态更新",closeEvent:function(){
		parent.elsDeskTop.showLoading("更新中");
		$.ajax({
			url :"<%=basePath%>rest/PurchaseBiddingService/roundsStatusUpdate",
			type :"POST",
			contentType : "application/json",
			dataType : "json",
			data:JSON.stringify(formObj),
