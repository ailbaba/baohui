...
<%--                 <button id="sendBtn" class="button-wrap"><i18n:I18n key="i18n_button_send" defaultValue="发送" /></button> --%>
        ...
        <div class="box-wrap pos-relative bg-common">
            ...
            <form id="queryForm" onsubmit="return false;" method="POST" action="<%=basePath%>rest/SaleQuotationService/findSaleQuotation">
...
<!--            <div class="dis-in-b input-and-tip"><span>供应商ELS: </span><input type="text" name="toElsAccount"/></div> -->
                <div id="div-orderSendStatus" class="dis-in-b input-and-tip">
                    <span class="dis-in-b text-right w-72"><i18n:I18n key="i18n_quotation_sendstatus" defaultValue="发送状态" />： </span>
...
var cols_head = [{title:"<i18n:I18n key='i18n_quotation_number' defaultValue='报价单号' />",name:"quotationNumber",align:"center",width:98},
                 ...
                 {title:"<i18n:I18n key='i18n_enquiry_title_sendstatus' defaultValue='发送状态' />",name:"sendStatus",align:"center",width:98,renderer:function(val){return dictionaryFormat(val,"0:<i18n:I18n key='i18n_label_notSent' defaultValue='未发送' />;1:<i18n:I18n key='i18n_label_sent' defaultValue='已发送' />")}},
...
var cols_file = [
                 ...
                 { title:'<i18n:I18n key="i18n_common_lable_materialnumber" defaultValue="物料编号" />', name:'orderItemNumber' ,width:150, align:'center'},
...
function modifyQuotation() {
    ...
    parent.elsDeskTop.defineWin({
        ...
        'windowTitle':'<i18n:I18n key="i18n_quotation_update" defaultValue="修改报价单" />',
        'iframSrc':'<%=basePath%>quotation/sale/editSaleQuotation.jsp?quotationNumber=' + quotationNumber + "&toElsAccount=" + headGrid.selectedRows()[0].toElsAccount,
        windowMinWidth: 960,
        windowMinHeight: 700,
...
function addQuotation() {
    ...
    parent.elsDeskTop.defineWin({
        'iconSrc':'icon/els-icon.png',
        'windowsId':'PurchaseOrderUpdate',
        'windowTitle':'<i18n:I18n key="i18n_quotation_chooselist" defaultValue="选择货源清单" />',
...
function loadFormFile() {
	...
	var url = "<%=basePath%>rest/SaleQuotationService/findAllFormFile";
	$.ajax({
		url : url,
		type : "post",
		dataType : "json",
...
function loadItem() {
	...
	var param={"elsAccount":elsAccount,"toElsAccount":headGrid.selectedRows()[0].toElsAccount,"quotationNumber":headGrid.selectedRows()[0].quotationNumber}
	var url = "<%=basePath%>rest/SaleQuotationService/findMaterialItem";
	$.ajax({
		url : url,
		type : "post",
		dataType : "json",
