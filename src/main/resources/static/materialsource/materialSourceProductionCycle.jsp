...
<%--             <button class="button-wrap" id="updBtn"><i18n:I18n key="i18n_button_update" defaultValue="修改" /></button> --%>
				 <button class="button-wrap" id="editBtn"><i18n:I18n key="" defaultValue="步骤详情" /></button>
...
<%-- 				<img class="dis-in-b" src="${pageContext.request.contextPath}/icon/min/search_gray.png" style="width: 20px; height: 20px;" id="queryToElsAccount" /> --%>
			...
			<div class="dis-in-b input-and-tip">
                 <span><i18n:I18n key="" defaultValue="是否维护" /></span>
...
<!-- 	                	</tr> -->
	                	...
	                	<tr class="line-height-tr">
	                		<td><i18n:I18n defaultValue="步骤简述" key=""/>:</td>
	                		<td><input type="text" id="fbk9" /></td>
	                	</tr>
	                	<tr class="line-height-tr">
	                		<td><i18n:I18n defaultValue="是否维护" key=""/>:</td>
	                		<td><input type="text" id="fbk10" /></td>
	                	</tr>
	                	<tr class="line-height-tr">
	                		<td><i18n:I18n defaultValue="周期合计" key=""/>:</td>
	                		<td><input type="text" id="fbk11" /></td>
	                	</tr>
<!-- 	                	<tr class="line-height-tr"> -->
...
var renderFbk10 = function(val,item,rowIndex){
	if("undefined"==typeof(val) || "null"==val){
		return "否";//默认启用
	} else if("0"==val){
		return "是";
...
var renderFbk11 = function(val,item,rowIndex){
...
// 	{ title:"<i18n:I18n key='' defaultValue='配额' />", name:"quota", width:100, align:"center"},
	{ title:"<i18n:I18n key='' defaultValue='步骤简述' />", name:"fbk9", width:250, align:"center"},
	{ title:"<i18n:I18n key='' defaultValue='是否维护' />", name:"fbk10", width:50, align:"center",renderer:renderFbk10},
	{ title:"<i18n:I18n key='' defaultValue='周期合计' />", name:"fbk11", width:50, align:"center",renderer:renderFbk11},
...
function editTemplate() {
	...
	var templateCode = headGrid.row(index[0]).materialNumber;
	var templateName = headGrid.row(index[0]).materialDesc;
	debugger;
    parent.elsDeskTop.defineWin({
        'iconSrc':'icon/els-icon.png',
        'windowsId':'editMaterialScheduleTemplate',
        'windowTitle':'<i18n:I18n key="" defaultValue="步骤详情" />',
        'iframSrc':'<%=basePath%>materialsource/editMaterialScheduleTemplate.jsp',
		data:headGrid.row(index[0]),
	...
	if(!excelUploader){
		...
		excelUploader.on( 'uploadAccept', function( obj, response ) {
			debugger;
		  	...
		  	$.ajax({
				url :"<%=basePath%>rest/MaterialSourceListManageService/importExcelByPath",
				type :"POST",
				contentType : "application/json",
				dataType : "json",
...
$("#downloadBtn").click(function(){
	...
	debugger;
	$.ajax({
		url :"<%=basePath%>rest/MaterialSourceListManageService/exportExcel",
		dataType : "json",
		type :"post",
		contentType : "application/json",
		data : JSON.stringify({"materialNumber":materialNumber,"materialDesc":materialDesc,"elsAccount":"160000","toElsAccount":toElsAccount,"fbk7":fbk7}),
		success : function(data) {
			parent.elsDeskTop.hideLoading();
