	...
	<script>
	 ...
	 var companyName = "<%=session.getAttribute("companyShortName")%>";
		...
		var initInputAny = function(val, item, index){
			if(!val) { val = '';}
			return '<input type="text" name="templateList[]['+this.name+']" id="'+this.name+'_'+index+'" value="'+val+'"/>';
		};
	  ...
	  $("#addBtn").click(function(e){
			 var cols={elsAccount:elsAccount,elsDesc:companyName,templateName:"",templateDesc:""};
			 headTable.addRows([cols]);
	  });
	  //新增
	 $("#saveBtn").click(function(e){
			var form =$("#iForm").serializeJSON();;
			var head = form.templateList;
			for(var i=0;i<head.length;i++){
				if(head[i].templateName==""){
					var num = i+1;
				 	alert('<i18n:I18n key="" defaultValue="请输入第['+num+']行名称" />',2);
					return;
				}
			...
			var frmData = JSON.stringify(head);
			  $.ajax({
				url : '<%=basePath%>rest/RegulationService/saveTemplateList',
				type : "POST",
				contentType : "application/json",
				data :frmData ,
				dataType : "json",
	 ...
	 var colst_head = [
	   	           // { title:'<i18n:I18n key="" defaultValue="编号" />', name:'regulationGroupNumber', width: 200, align: 'center'},
	   	        	...
	   	        	{ title:'<i18n:I18n key="" defaultValue="名称" />', name:'templateName' ,width:200, align:'center', renderer:initInputAny},
	   	        	...
	   	        	{ title:'<i18n:I18n key="" defaultValue="描述" />', name:'templateDesc' ,width:200, align:'center' ,renderer:initInputAny},
