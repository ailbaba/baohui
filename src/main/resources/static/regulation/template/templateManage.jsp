	...
	<div class="pos-relative public-wrap">
		<div class="box-wrap pos-relative bg-common">
			<input type="hidden" id="elsAccount" name="elsAccount"
				value="<%=session.getAttribute("elsSubAccount")%>" /> <span
				class="dis-in-b pos-absolute box-tip bg-common"><i18n:I18n
					key="i18n_common_title_baseoperation" defaultValue="基本操作" /></span>
			<div class="common-box-line">
				<button class="button-wrap" id="queryHeadBtn">
		...
		<form id="queryForm">
			<div class="box-wrap pos-relative bg-common">
				<span class="dis-in-b pos-absolute box-tip bg-common"> <i18n:I18n
						key="i18n_common_title_conditionquery" defaultValue="条件查询" />
				</span>
				<div class="common-box-line">
					<div class="dis-in-b input-and-tip">
						<span>编号:</span> <input type="text" id="templateNumber"
							name="templateNumber" />
					</div>
					<div class="dis-in-b input-and-tip">
					...
					<div class="dis-in-b input-and-tip">
						<span><i18n:I18n key="i18n_common_title_createuser"
								defaultValue="创建人" />:</span> <input type="text" id="createUser"
							name="createUser" />
					</div>
					<div class="dis-in-b input-and-tip">
						<span><i18n:I18n key="i18n_sampleManage_createDate"
								defaultValue="创建日期" />:</span> <input type="radio" name="dateArea"
	...
	<script>
	 ...
	 // 渲染   @author jiangzhidong 20160120  begin
		...
		var initOpt = function(val, item, index){
			return '<span style = "color:blue;cursor:pointer"  class="findTemplate" ><i18n:I18n key="" defaultValue="详情" /> </span><span style="display: none;">'+JSON.stringify(item)+'</span>';
		};
		
		var initParticipants = function(val,item,index) {
			if (val == "purchese"){
				return '<i18n:I18n key="test" defaultValue="采购方" />';
			}else if (val == "sale") {
				return '<i18n:I18n key="test" defaultValue="销售方" />';
			}
		};
		...
		$('.findSelection').live("click", function() {
			...
			parent.elsDeskTop.defineWin({
		        ...
		        iframSrc : "${pageContext.request.contextPath}/regulation/regulationSelect.jsp?pid=regulationTemplate&regulationNumber=" + headRows.regulationNumber,
		        showAdv : true,
		        windowMinWidth : 800,
		        windowMinHeight : 600,
		        windowWidth : 800,
	 ...
	 $("#addBtn").click(function(){
				parent.elsDeskTop.defineWin({
			        iconSrc : "icon/icon9.png",
			        windowsId : "addTemplate",
			        windowTitle : "<i18n:I18n defaultValue='添加模版' key=''/>",
			        iframSrc : "${pageContext.request.contextPath}/regulation/template/addTemplateList.jsp",
			        showAdv : true,
			        showAdv : true,
			        windowMinWidth : 600,
			        windowMinHeight : 500,
	 ...
	 $("#updBtn").click(function(){
	 	   ...
	 	   if(isReference(head[0])){
			 	alert('<i18n:I18n key="test" defaultValue="此模板已经被准入引用不能修改!" />',2);
			 	return;
			}
	 	    var number=head[0].templateNumber;
	 	    var name=head[0].templateName;
	 	    var desc=head[0].templateDesc;
	 	    var rNumber=head[0].regulationNumber;
	 	    var url = "${pageContext.request.contextPath}/regulation/template/editTemplate.jsp?templateNumber="+
	 	   number+"&templateName="+name+"&templateDesc="+desc+"&regulationNumber="+rNumber;
				parent.elsDeskTop.defineWin({
			        iconSrc : "icon/icon9.png",
			        windowsId : "editTemplate",
			        windowTitle : "<i18n:I18n defaultValue='修改模版' key=''/>",
			        iframSrc :url ,
			        showAdv : true,
	 ...
	 $('.findTemplate').live("click", function() {
			var editCol = $(this).next().text();
			var head = JSON.parse(editCol);
			var number=head.templateNumber;
	 	    var name=head.templateName;
	 	    var desc=head.templateDesc;
	 	    var rNumber=head.regulationNumber;
	 	    var url = "${pageContext.request.contextPath}/regulation/template/editTemplate.jsp?templateNumber="+
	 	   number+"&templateName="+name+"&templateDesc="+desc+"&regulationNumber="+rNumber;
				parent.elsDeskTop.defineWin({
			        iconSrc : "icon/icon9.png",
	 ...
	 function isReference(head){
	 		var bool = false;
		 	$.ajax({
	 			url : '<%=basePath%>rest/RegulationService/iSsupplyTemplate',
	 			type : 'POST',
	 			async:false,
	 ...
	 $("#copyAddBtn").click(function(){
	 	    ...
	 	    var desc=head[0].templateDesc;
	 	    var rNumber=head[0].regulationNumber;
	 	    var url = "${pageContext.request.contextPath}/regulation/template/editTemplate.jsp?cy=1&templateNumber="+
	 	   number+"&templateName="+name+"&templateDesc="+desc+"&regulationNumber="+rNumber;
				parent.elsDeskTop.defineWin({
			        iconSrc : "icon/icon9.png",
			        windowsId : "editTemplate",
		...
		$("#delBtn").click(function(){
			 var head = headTable.selectedRows();
			 if(isReference(head[0])){
				 	alert('<i18n:I18n key="test" defaultValue="此模板已经被准入引用不能删除!" />',2);
				 	return;
			}
			 if(head.length<=0){
			 ...
			 parent.elsDeskTop.tip.init({type: 'confirm',message:'<i18n:I18n key="i18n_common_confirm_delete" defaultValue="是否确认删除？" />',closeEvent:function(){
				    ...
				    var param=head[0];
			 		 $.ajax({
			 			url : '<%=basePath%>rest/RegulationService/deleteTemplate',
			 			type : 'POST',
			 			contentType : "application/json",
