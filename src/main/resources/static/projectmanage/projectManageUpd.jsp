...
var headEditCols = function(val,item,rowIndex){
	...
	if("elsAccount" == colName || "elsDesc" == colName){
		return "<input typpe='text' name='"+colName+"' class='headColsEdit' readonly='readonly' value='"+val+"' />";
	} else if("projectStatus" == colName){
		var stsSelect = projectStatusSelect.clone();
		stsSelect.find("option[value='"+val+"']").attr("selected",true);
		return stsSelect.prop("outerHTML");
	} else if("auditStatus" == colName){
		var approvSelect = auditStatusSelect.clone();
		approvSelect.find("option[value='"+val+"']").attr("selected",true);
		return approvSelect.prop("outerHTML");
	} else if("planStartDate"==colName){
		return "<input typpe='text' name='"+colName+ "' id='plans'" +
		" class='Wdate' onfocus=\"WdatePicker({minDate:'%y-%M-%d',maxDate:'#F{$dp.$D(\\'plane\\')}'})\" value='"+new Date(val).format("yyyy-MM-dd")+"' />";
	} else if("planEndDate"==colName){
		return "<input typpe='text' name='"+colName+ "' id='plane'" +
		" class='Wdate' onfocus=\"WdatePicker({minDate:'#F{$dp.$D(\\'plans\\')}'})\" value='"+new Date(val).format("yyyy-MM-dd")+"' />";
	}else if("actualStartDate"==colName){
		return "<input typpe='text' name='"+colName+ "' id='actuals'" +
		" class='Wdate' onfocus=\"WdatePicker({minDate:'%y-%M-%d',maxDate:'#F{$dp.$D(\\'actuale\\')}'})\" value='"+new Date(val).format("yyyy-MM-dd")+"' />";
	}else if("actualEndDate"==colName){
		return "<input typpe='text' name='"+colName+ "' id='actuale'" +
		" class='Wdate' onfocus=\"WdatePicker({minDate:'#F{$dp.$D(\\'actuals\\')}'})\" value='"+new Date(val).format("yyyy-MM-dd")+"' />";
	} else {
...
var headCols = [
    { title:"ELS号", name:"elsAccount", width:100, align:"center",renderer:headEditCols,hidden:true},
    { title:"ELS描述", name:"elsDesc", width:100, align:"center",renderer:headEditCols,hidden:true},
    ...
    { title:"项目状态", name:"projectStatus" , width:100, align:"center",renderer:headEditCols},
    ...
    { title:"实际金额", name:"actualAmount" , width:100, align:"center",renderer:headEditCols},
    { title:"计划周期(天)", name:"planCycle" , width:100, align:"center",renderer:headEditCols},
    { title:"实际周期(天)", name:"actualCycle" , width:100, align:"center",renderer:headEditCols},
    ...
    { title:"实际开始日期", name:"actualStartDate" , width:100, align:"center",renderer:headEditCols},
    { title:"实际结束日期", name:"actualEndDate" , width:100, align:"center",renderer:headEditCols},
...
headGrid.on("loadSuccess",function(e, data){
	if(headGrid.rowsLength()>0){
		...
		var headRow = headGrid.rows()[0];
		headRow["projectNumberTemp"] = headRow.projectNumber;
	} else {
		clearItemTable();
		clearContactTable();
		clearFileTable();
...
var itemEditCols = function(val,item,rowIndex){
	...
	if("elsAccount" == colName || "elsDesc" == colName || "projectNumber"==colName || "budgetPercentage"==colName){
		return "<input typpe='text' name='"+colName+"' class='itemColsEdit' readonly='readonly' value='"+val+"' />";
	} else if("projectItemStatus" == colName){
		var stsSelect = itemStatusSelect.clone();
		stsSelect.find("option[value='"+val+"']").attr("selected",true);
		return stsSelect.prop("outerHTML");
		...
		" class='Wdate' onfocus=\"WdatePicker({minDate:'%y-%M-%d',maxDate:'#F{$dp.$D(\\'plane_"+rowIndex+"\\')}'})\" value='"+new Date(val).format("yyyy-MM-dd")+"' />";
	} else if("planEndDate"==colName){
		return "<input typpe='text' name='"+colName+ "' id='plane_" + rowIndex + "'" +
		" class='Wdate' onfocus=\"WdatePicker({minDate:'#F{$dp.$D(\\'plans_"+rowIndex+"\\')}'})\" value='"+new Date(val).format("yyyy-MM-dd")+"' />";
	}else if("actualStartDate"==colName){
		return "<input typpe='text' name='"+colName+ "' id='actuals_" + rowIndex + "'" +
		" class='Wdate' onfocus=\"WdatePicker({minDate:'%y-%M-%d',maxDate:'#F{$dp.$D(\\'actuale_"+rowIndex+"\\')}'})\" value='"+new Date(val).format("yyyy-MM-dd")+"' />";
	}else if("actualEndDate"==colName){
		return "<input typpe='text' name='"+colName+ "' id='actuale_" + rowIndex + "'" +
		" class='Wdate' onfocus=\"WdatePicker({minDate:'#F{$dp.$D(\\'actuals_"+rowIndex+"\\')}'})\" value='"+new Date(val).format("yyyy-MM-dd")+"' />";
	} else {
...
$(".itemColsEdit").live('keyup',function(){
	...
	} else if("actualAmount"==thisName){
		reCalcActualAmount();
	}
});
var itemCols = [
    { title:"ELS号", name:"elsAccount", width:100, align:"center",renderer:itemEditCols,hidden:true},
    { title:"ELS描述", name:"elsDesc", width:100, align:"center",renderer:itemEditCols,hidden:true},
    ...
    { title:"项目行状态", name:"projectItemStatus" , width:100, align:"center",renderer:itemEditCols},
    ...
    { title:"实际金额", name:"actualAmount" , width:100, align:"center",renderer:itemEditCols},
    { title:"计划周期(天)", name:"planCycle" , width:100, align:"center",renderer:itemEditCols},
    { title:"实际周期(天)", name:"actualCycle" , width:100, align:"center",renderer:itemEditCols},
    ...
    { title:"实际开始日期", name:"actualStartDate" , width:100, align:"center",renderer:itemEditCols},
    { title:"实际结束日期", name:"actualEndDate" , width:100, align:"center",renderer:itemEditCols},
...
$(document).ready(function(){
    ...
    var queryElsAccount = "${param.elsAccount}";
    var queryProjectNumber = "${param.projectNumber}";
    var queryParam = {
    		elsAccount : queryElsAccount,
    		projectNumber : queryProjectNumber,
    };
...
$("#addItemRowBtn").click(function(){
	...
	var newItemRecord = {
			...
			actualStartDate : currDate.format("yyyy-MM-dd"),
			actualEndDate : currDate.format("yyyy-MM-dd"),
...
$("#saveBtn").click(function(){
	...
	$.ajax({
		url :"${pageContext.request.contextPath}/rest/ProjectManageService/updProjectManage",
		type :"post",
		...
		success : function(data) {
			...
			if(data.statusCode=="-200"){
				...
				parent.elsDeskTop.tip.init({type: 'confirm',message:'<i18n:I18n key="i18n_common_confirm_returnParentPage" defaultValue="是否返回上级页面？" />',closeEvent:function(){
					parent.elsDeskTop.closeCurWin("projectManageUpd",callBack);
				}});
			}
		},
		error : function(data) {
