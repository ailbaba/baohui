...
<div class="pos-relative public-wrap">
    <form id="requirementsplanningForm" onsubmit="return false;" onkeydown="if(event.keyCode==13)return false;">
		...
		<input type="hidden" id="businessElsAccountId" name="businessElsAccount" value="<%=businessElsAccount%>">          
            ...
            <%--
             ...
             --%>
				<div>
					...
					<base:audit elsSubAccount="${elsSubAccount }" businessElsAccount="${param.businessElsAccount }" businessId="<%=com.els.util.Base64.getFromBase64(businessKey) %>" elsAccount="${elsAccount }" businessType="requirementplan">
		...
		<%--审批流 --%>
		<div class="fixed-dialog" id="auditFixedDialog">
				<div class="dialog-tip bg-common">
					<div class="dialog-close-btn" id="dialogAuditClose"></div>
					<div class="tip-head">
						<img class="icon" src="<%=basePath %>icon/els.PNG" /> <span
...
var businessKey = decode("<%=businessKey%>");
$("#businessKey").val(businessKey);
$("#stretegyCode").val(businessKey);
var jsonBusinessKey = JSON.parse(businessKey);
var requirementsPlanningNumber = jsonBusinessKey.rpNO;
var projectNumber = jsonBusinessKey.pNO;
var projectItemNumber = jsonBusinessKey.piNO;
...
function materialAttrWindow(item,rowIndex) {//查看物料属性
    ...
    parent.elsDeskTop.defineWin({
        ...
        iframSrc : "${pageContext.request.contextPath}/materialmanage/fillMaterialAtrrValue.jsp?elsAccount="+elsAccount+"&materialNumber="+materialNumber+"&rowIndex="+rowIndex+"&operationType="+operationType+"&windowsId=auditRecord",
        showAdv : false,
 	   	mask : true,
...
$("#save-requirementsplanning").click(function() {
	saveRequirementsplanning();
});

function saveRequirementsplanning() {
    var rows = headGrid.rows();
    var retMsg = '';
    if(rows == null || headGrid.rowsLength() == 0){
        return'<i18n:I18n key="i18n_requirementsPlanning_alert_atLeastOneItemIsAdded" defaultValue="至少添加一条物料信息！"/>';
    }  else {
...
function selectRequirementTypeCode(item,rowIndex){
    ...
    parent.elsDeskTop.defineWin({
        ...
        'windowSubTitle': companyShortName,
        'iframSrc':'${pageContext.request.contextPath}/requirement/requirementtype/requirementTypeCheck.jsp',
        'showAdv': false,
 	   	'mask' : true,
        'windowStatus': 'regular',
        'windowMinWidth': 1024,
...
function selectMaterialNumber(item,rowIndex){
    ...
    parent.elsDeskTop.defineWin({
        ...
        'windowSubTitle': companyShortName,
        'iframSrc':'${pageContext.request.contextPath}/materialmanage/materialCheck.jsp',
        'showAdv': false,
 	   	'mask' : true,
        'windowStatus': 'regular',
        'windowMinWidth': 1024,
...
$("#dialogAuditClose").click(function(){
    $("#auditFixedDialog").hide();
});
function pass() {
	parent.elsDeskTop.tip.init({type: 'confirm',message:'<i18n:I18n key="i18n_common_confirm_auditpass" defaultValue="确认要审批通过吗？" />',closeEvent:function(){
		...
		$.ajax({
			url :"<%=basePath%>rest/RequirementsPlanningService/sendPass",
			type :"post",
			contentType : "application/json",
			...
			success : function(data) {
				...
				// 保存修改内容
				var alertMsg = saveRequirementsplanning();
				if(alertMsg=='') {
					$("#passBtn").hide();
					$("#noPassBtn").hide();
					$("#cancelBtn").show();
					alert("<i18n:I18n key="i18n_common_alert_successfulOperation" defaultValue="操作成功" />");
					window.location.href = "<%=basePath%>projectmanage/requirementsplanning/auditRequirementsPlanning.jsp?businessElsAccount=" + $("#businessElsAccountId").val()
						+ "&businessKey=" + encode($("#businessKey").val())
						+ "&stretegyCode="+$("#stretegyCode").val()
						+ "&flowCode=done";
				} else {
					alert(alertMsg, 2);
				}
...
function nopass() {
	parent.elsDeskTop.tip.init({type: 'confirm',message:'<i18n:I18n key="i18n_common_confirm_auditrefuse" defaultValue="确认要审批拒绝吗？" />',closeEvent:function(){
		...
		var frm = $("#requirementsplanningForm");
		$.ajax({
			url :"<%=basePath%>rest/RequirementsPlanningService/sendNoPass",
			type :"post",
			contentType : "application/json",
			data:JSON.stringify(frm.serializeJSON()),
			dataType : "json",
...
function flow(){
	...
	var businessType = "requirementplan";
	var businessId = "<%=businessKey%>";
	parent.elsDeskTop.defineWin({
        'iconSrc':'icon/els-icon.png',
        ...
        windowSubTitle: "",
 	   	mask : true,
        'iframSrc':'<%=basePath%>audit/auditFlowList.jsp?elsAccount=' + targetElsAccount
