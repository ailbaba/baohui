...
var requirementsPlanningHeadCols = [
        ...
        { title:'<i18n:I18n key="i18n_requirementsPlanning_lable_approvalTime" defaultValue="审批时间"/>', name:'approveDate' , width:120, align:'center',renderer:headRender},
...
$(".previewImage-item").live("click", function() {
    ...
    if("undefined"!=typeof(imageList) && imageList.length > 0){
		...
		for(var i=0;i<imageList.length;i++) {
			imageList[i]["preViewFilePath"] = imageList[i].requirementFilePath;
		}
        var obj={
                windowTitle: "图片预览",
                data:{
...
function init(){
    ...
    itemGrid=$('#itemTable').mmGrid({
        ...
        url: '${pageContext.request.contextPath}/rest/RequirementsPlanningService/findRecItemList',
        params: param,
        height: 300,
        method: 'post',
        root: 'rows',
...
$("#saveBtn").click(function() {
        ...
        for(var i=0;i<itemGrid.rowsLength();i++){
            ...
            if("undefined"!=typeof(materialAttrList) && materialAttrList.length > 0){
                for(var j=0;j<materialAttrList.length;j++){
                    var attrRecord = {
                            elsAccount : elsAccount,
                            toElsAccount : itemRow.toElsAccount,
    ...
    $.ajax({
        url : "${pageContext.request.contextPath}/rest/RequirementsPlanningService/updateRec",
        type : "POST",
        contentType : "application/json",
        dataType : "json",
...
$("#sendBtn").click(function() {
    ...
    parent.elsDeskTop.showLoading('<i18n:I18n key="i18n_requirementsPlanning_alert_inTheSave" defaultValue="保存中..." />');
    $.ajax({
        url : "${pageContext.request.contextPath}/rest/RequirementsPlanningService/sendRec",
        type : "POST",
        contentType : "application/json",
        dataType : "json",
        data:JSON.stringify(headRow),
