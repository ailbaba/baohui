	    ...
	    <div>
	    	 <div class="box-wrap pos-relative bg-common" >
				...
				<div class="dis-in-b input-and-tip">
					...
					<input type="text" id="businessTypeQuery" name="businessTypeQuery" />
				</div>
				<br>
                <div class="dis-in-b input-and-tip">
...
<script>
	...
	function init() {
	    ...
	    var headData = [{
	    	...
	    	templateDesc : "",
	    	templateVersion : "",
	    	//contractSignedDate : currDate.format("yyyy-MM-dd"),
	    	currency : "CNY",
	    	contractStatus : "0",
	    	sendStatus : "0",
	    	auditStatus : "1",
	    	purchasePerson : elsSubAccount+"_"+userName,
	    }];
	    
	    headGrid.load(headData);
	}
    ...
    $("#saveBtn").click(function(){
		...
		$.ajax({
			...
			success : function(data) {
				...
				parent.elsDeskTop.closeCurWin("purchaseContractAdd",callBack);
			},
			error : function(data) {
				parent.elsDeskTop.hideLoading();
	...
	function bussinessTypeSelect(val,item,rowIndex) {
        var sel = '<div><p style="display: none;">'+JSON.stringify(item)+'</p><select name="businessType" id="businessType" class="select-tag" style="padding: 0;height: 25px;width: 100px;">';
        if(!val){
      	  sel += '<option value="" selected>请选择</option><option value="0">机械</option><option value="1">电商</option><option value="2">农业</option></select>';
        }
      return sel;
  	}
	//TODO:设置阶梯报价下拉事件
