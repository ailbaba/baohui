...
<body>
    <div class="pos-relative public-wrap">
        ...
        <div class="box-wrap pos-relative bg-common">
            <span class="dis-in-b pos-absolute box-tip bg-common"><i18n:I18n defaultValue="销售合同头信息" key=""/></span>
            <div class="common-box-line">
        ...
        <div class="box-wrap pos-relative bg-common">
            <span class="dis-in-b pos-absolute box-tip bg-common"><i18n:I18n defaultValue="销售合同详细信息" key=""/></span>
            <div class="common-box-line">
...
<script>
	...
	headGrid.on('cellSelected',function(e, item, rowIndex, colIndex){
		...
		if(_this.hasClass("templateNumber")) {
			showTemplateNumber();
		} else {
   ...
   /*  fileGrid.on("cellSelected", function(e, item, rowIndex, colIndex){
    ...
    $("#saveBtn").click(function(){
		var saleContractHead = headGrid.row(0);
	   	var saleContractClause = clauseGrid.rowsLength() == 0 ? [] : clauseGrid.rows();
	   	var saleContractMaterial = materialGrid.rowsLength() == 0 ? [] : materialGrid.rows();
	   	var saleContractFile = fileGrid.rowsLength() == 0 ? [] : fileGrid.rows();
	   	
	   	var data = {
	   			operate : "save",
	   			saleContractHead : JSON.stringify(saleContractHead),
	   			saleContractClause : JSON.stringify(saleContractClause),
	   			saleContractMaterial : JSON.stringify(saleContractMaterial),
	   			saleContractFile : JSON.stringify(saleContractFile),
	   	}
	   	
	   	parent.elsDeskTop.showLoading("<i18n:I18n defaultValue='保存中' key=''/>");
		$.ajax({
			url :"${pageContext.request.contextPath}/rest/ContractManageService/saveSaleContract",
			type :"post",
			contentType : "application/json",
			...
			success : function(data) {
				...
				parent.elsDeskTop.closeCurWin("saleContractUpd",callBack);
			},
			error : function(data) {
				parent.elsDeskTop.hideLoading();
	...
	$("#saveSendBtn").click(function(){
		var saleContractHead = headGrid.row(0);
	   	...
	   	parent.elsDeskTop.showLoading("<i18n:I18n defaultValue='销售合同发送中!' key=''/>");
		$.ajax({
			url :"${pageContext.request.contextPath}/rest/ContractManageService/saveSaleContract",
			type :"post",
			contentType : "application/json",
	...
	function callBack(){
		if(parent.frames['iframeApp_saleContractManage']) {
			parent.frames['iframeApp_saleContractManage'].init();
		}
	}
	...
	$("#queryBasicClauseDialogOkBtn").click(function(){
		...
		if("undefined" != typeof(clauseRows) && clauseGrid.rowsLength() > 0 && "undefined" != typeof(selectRows)){
			for( var i = 0 ; i < clauseRows.length ; i ++){
				...
				if("undefined" != typeof(clauseRow) && clauseRow.length > 0){
