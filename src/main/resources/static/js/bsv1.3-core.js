			...
			[{
				...
				func: function() {Windows.openSys({
						...
						[{
							text:"注销",
							func:function(){
								location.href="login/logout.jsp?t="+new Date().getTime();
							}
						}]
					];
			return MenuData;
	...
	return me ={
		 ...
		 bindTheme:function(){
			 ...
			 $("a",themsSetting).click(function(){
					...
					var h = $(window).height();
					var w = $(window).width();
					$("#zoomWallpaper").attr("src","images/"+src+".jpg").width(w).height(h);
...
ElsePanel = function(me){
	...
	var inform="<div id=\"inform\"><div class=\"infotitl\">通知</div>&nbsp;你有新订单</div>";
	var _msgBox = "<div id=\"msg\"><div id=\"msg_close\"><a href=\"#\">关闭</a></div></div>";
	
	var chat="<div class=\"chatRight\"><div class=\"chat03\"><div class=\"chat03_title\"><label class=\"chat03_title_t\">成员列表</label></div><div class=\"chat03_content\"><ul><li><label class=\"online\"></label><a href=\"javascript:;\"><img src=\"img/head/2013.jpg\"></a><a href=\"javascript:;\" class=\"chat03_name\">诺普信</a></li></ul> </div></div></div>";
...
appIcon_t1 = appIcon_t0.extend({		
	...
	create:function(fid){
		...
		this.box.attr({
			...
			title:this.app.name,
			url:this.app.url,
			fullscreen:this.app.fullscreen,
	...
	bindEvent:function(){//绑定事件
		this.box.click(function(e){
			...
			var title = $.trim(_this.text());
			var url =_this.attr("url");
			var fullscreen =_this.attr("fullscreen");
			...
			if(fullscreen)
				Windows.openApp(id,title,url,icon,jsonSonMenu,'88%','85%');
			else
				Windows.openApp(id,title,url,icon,jsonSonMenu,700,500);
