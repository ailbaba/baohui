	     ...
	     <div class="dis-in-b input-and-tip">
			<table>
				...
				<tr>
					...
					<td><i18n:I18n defaultValue="采购方负责人" key=""/>:</td>
					<td>
...
$(document).ready(function(){
	...
	$("#saveBtn").click(function(){
		var additionCostNumber = $("#additionCostNumber").val();
	   	...
	   	if("undefined"==typeof(costDate) || "" == costDate){
			  ...
			  type : 'alert',
			  displayType : 3,
			  message : "<i18n:I18n defaultValue='请选择费用日期' key='i18n_label_additionCost_pleaseSelectCostDate'/>"
			});
	   		return;
	   	}
	   	...
	   	if("undefined"==typeof(currency) || "" == currency){
			  ...
			  type : 'alert',
			  displayType : 3,
			  message : "<i18n:I18n defaultValue='请选择币种' key='i18n_label_pleaseSelectCurrencyType'/>"
			});
	   		return;
	   	}
		...
		$.ajax({
			url :"${pageContext.request.contextPath}/rest/PurchaseAdditionCostService/addPurchaseAdditionCost",
			type :"post",
			...
			success : function(data) {
				...
				if (data&&data.statusCode == "-100") {
					 ...
					 alert("<i18n:I18n defaultValue='保存成功！' key='i18n_common_alert_savesuccess'/>",1);
						...
						parent.elsDeskTop.closeCurWin('purchaseAdditionCostAdd',callBack);
					 }});
				}
			},
			error : function(data) {
	...
	$("#saveSendBtn").click(function(){
		var additionCostNumber = $("#additionCostNumber").val();
	  	 ...
	  	 $.ajax({
				...
				error : function(data) {
					parent.elsDeskTop.hideLoading();
					alert("<i18n:I18n defaultValue='保存发布出错！' key='test'/>",1);
				}
			});
	...
	var toElsTable = $('#selectFriendElsAccountTable').mmGrid({
	                ...
	                params : {
	                	...
	                	"groupIdAName" : "<i18n:I18n defaultValue='供应商' key='i18n_enquiry_label_supplier'/>",
	                	"pageSize" : 1000,
	                },
	                method : "post",
	                root : "rows",
	...
	var queryPurchaseOrderCols = [
            ...
            { title:"<i18n:I18n defaultValue='物料描述' key='i18n_common_title_materialdescribe'/>", name:'materialDesc' ,width:100, align:'center'},
	...
	$("#queryPurchaseOrderDialogOkBtn").click(function(){
		...
		for(var i = 0 ; i < selectRows.length ; i++ ){ //设置初始值
			...
			var record = {
					...
					materialNumber : selectRow.materialNumber,
					materialDesc : selectRow.materialDesc,
					companyCode : "",
