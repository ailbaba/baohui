...
$(document).ready(function(){
	...
	$("#invoiceDelBtn").click(function(){
		...
		if(busReconStatus=='4'||busReconStatus=='5'){
			alert('<i18n:I18n defaultValue="发票全部确认,不能操作!" key="test"/>',2);
			return;
		}
		if(selectedRowsIndex.length<=0){
			alert('<i18n:I18n defaultValue="请选择删除行!" key="test"/>',2);
			return;
		}
		debugger;
	...
	$("#invoiceSaveBtn").click(function(){
		// 发票总金额=收货总金额+附加费总金额-扣款总金额 @author jiangzhidong @date 20170117 begin
		var saleBusReconHead = headGrid.rowsLength() == 0 ? [] : headGrid.rows();
		var saleBusReconInvoice = invoiceGrid.rowsLength() == 0 ? [] : invoiceGrid.rows();
		var data = {
			saleBusReconHead : JSON.stringify(saleBusReconHead[0]),
			saleBusReconInvoice : JSON.stringify(saleBusReconInvoice)
		}
		// 发票总金额=收货总金额+附加费总金额-扣款总金额 @author jiangzhidong @date 20170117 end
	...
	// 处理行金额-总发票金额,总税金额 @author jiangzhidong @date 20170111 begin
	...
	// 处理行金额-总发票金额,总税金额 @author jiangzhidong @date 20170111 end
	//------------- 发票相关---------- end ----------------
