	     ...
	     <div class="dis-in-b input-and-tip">
			<table>
				...
				<tr>
					...
					<td><i18n:I18n defaultValue="对方ELS号" key="i18n_label_additionCost_toElsAccount"/>：</td>
					<td>
					...
					<td><i18n:I18n defaultValue="对方ELS描述" key="i18n_additionCost_toElsDesc"/>：</td>
					<td><input type="text" id="toElsDesc" name="toElsDesc" /></td>
					<td><i18n:I18n defaultValue="本方负责人" key="i18n_common_title_responserofsupplier"/>：</td>
					<td>
						<input type="hidden" id="additionCostType" name="additionCostType" value="add">
...
<div class="tab-wrap tab-wrap-change" id="add-enq-sheet">
    ...
    <div class="p-zero" id="edit-line-one" style="padding: 1px;">
        <div class="edit-box-wrap" style="padding: 1px;">
            <button id="querySaleOrder" class="button-wrap"><i18n:I18n defaultValue="选择订单" key="i18n_enquiry_SelectTheOrder"/></button>
    ...
    <div class="p-zero" id="edit-line-two">
		            <div class="common-box-line p-zero" style="padding: 1px;">
...
<div class="fixed-dialog" id="querySaleOrderDialog" >
	<div class="dialog-tip bg-common" style="width: 900px;height: 533px;margin-left: -415px;margin-top: -270px;">
	    <div class="dialog-close-btn" id="querySaleOrderDialogClose"></div>
        <div class="tip-head">
	        <img class="icon" src="../../icon/els.PNG" />
	        <span class="dialogTitle" id="querySaleOrderDialogTitle"><i18n:I18n defaultValue="销售方订单查询" key="i18n_additionCost_salesOrderInquiry"/></span>
	    </div>
	    <div class="" >
	        <div class="box-wrap pos-relative bg-common" >
	            ...
	            <div class="common-box-line">
	                <table>
	                	<tr class="line-height-tr">
	                	    <td  class="tb-p"><i18n:I18n defaultValue="销售单号" key="i18n_label_saleOrderNumber"/>:</td>
	                		<td><input class="easyui-textbox" style="width: 100px;" type="text" id="saleOrderNumber" /></td>
							...
							<td><button id="querySaleOrderBtn" class="button-wrap "><i18n:I18n defaultValue="查询" key="i18n_common_button_query"/></button></td>
	                	</tr>
	                </table>
	            </div>
	        </div>
	        <div class="box-wrap pos-relative bg-common" >
	            <main class="bg-common p-6">
                    <div class="grid-100" >
                        <table id="querySaleOrderTable"></table>
                    </div>
                    <div class="mt-20 text-right" >
	                    <button id="querySaleOrderDialogOkBtn" class="button-wrap "><i18n:I18n defaultValue="确定" key="i18n_common_button_ok"/></button>
	                    <button id="querySaleOrderDialogCancelBtn" class="button-wrap tip-option-btn"><i18n:I18n defaultValue="取消" key="i18n_common_button_cancel"/></button>
	                </div>
                 </main>
	        </div>
	    </div>
...
$(document).ready(function(){
    ...
    $("#costDate").val(currDate.format('yyyy-MM-dd'));
	...
	$("#saveBtn").click(function(){
		var additionCostNumber = $("#additionCostNumber").val();
	   	...
	   	var saleAdditionCostItem = itemGrid.rows();
	   	for(var i=0;i<saleAdditionCostItem.length;i++){
	   		if(isNaN(saleAdditionCostItem[i].additionCostAmount)){
	   			alert("<i18n:I18n defaultValue='金额非数字类型，不能保存' key=''/>",2);
		   		return;
	   		}
	   		if(saleAdditionCostItem[i].additionCostAmount <= 0 ){
	   			alert("<i18n:I18n defaultValue='金额不能小于0' key=''/>",2);
		   		return;
	   		}
	   		saleAdditionCostItem[i]["toElsAccount"] = toElsAccount;
	   		saleAdditionCostItem[i]["currency"] = currency;
	   	}
	   	
	   	var saleAdditionCostFile = [];
	   	if(fileGrid && fileGrid.rowsLength()){
	       	saleAdditionCostFile = fileGrid.rows();
	       	for(var i=0;i<saleAdditionCostFile.length;i++){
	       		saleAdditionCostFile[i]["toElsAccount"] = toElsAccount;
	       		saleAdditionCostFile[i]["currency"] = currency;
	       		saleAdditionCostFile[i]["fileSequence"] = i+1;
	       	}
	   	}
	   	
	   	
	   	var data = {
	   			saleAdditionCostHead : JSON.stringify(saleAdditionCostHead),
	   			saleAdditionCostItem : JSON.stringify(saleAdditionCostItem),
	   			saleAdditionCostFile : JSON.stringify(saleAdditionCostFile),
	   	}
	   	
	   	parent.elsDeskTop.showLoading("<i18n:I18n defaultValue='费用单保存中！' key='i18n_label_additionCost_additionCostIsSaved'/>");
		$.ajax({
			url :"${pageContext.request.contextPath}/rest/SaleAdditionCostService/addSaleAdditionCost",
			type :"post",
			...
			success : function(data) {
				...
				parent.elsDeskTop.tip.init({type: 'confirm',message:'<i18n:I18n key="i18n_common_confirm_returnParentPage" defaultValue="是否返回上级页面？" />',closeEvent:function(){
					parent.elsDeskTop.closeCurWin('saleAdditionCostAdd',callBack);
				}});
			},
			error : function(data) {
				parent.elsDeskTop.hideLoading();
	...
	$("#saveSendBtn").click(function(){
		var additionCostNumber = $("#additionCostNumber").val();
	   	...
	   	parent.elsDeskTop.showLoading("<i18n:I18n defaultValue='费用单保存发布中！' key='test'/>");
	  	 $.ajax({
				url :"${pageContext.request.contextPath}/rest/SaleAdditionCostService/sendSaleAdditionCost",
				type :"post",
				contentType : "application/json",
				data : JSON.stringify(data),
				dataType : "json",
				...
				error : function(data) {
					parent.elsDeskTop.hideLoading();
					alert("<i18n:I18n defaultValue='保存发布出错！' key='test'/>");
				}
			});
	...
	function callBack(){
		if(parent.frames['iframeApp_saleAdditionCostManage']) {
			parent.frames['iframeApp_saleAdditionCostManage'].init();
		}
	} 
	...
	var querySaleOrderCols = [
            { title:"<i18n:I18n defaultValue='销售订单号' key='i18n_label_saleOrderNumber'/>", name:'saleOrderNumber', width: 100, align: 'center'},
            ...
            { title:"<i18n:I18n defaultValue='物料编码' key='i18n_common_title_materialnumber'/>", name:'purchaseMaterialNumber' ,width:100, align:'center'},
            { title:"<i18n:I18n defaultValue='物料描述' key='i18n_common_title_materialdescribe'/>", name:'purchaseMaterialDesc' ,width:100, align:'center'},
	...
	var querySaleOrderGrid;
	//查询销售订单显示
	$("#querySaleOrder").click(function(){
		var toElsAccount = $("#toElsAccount").val();
		if("undefined"==typeof(toElsAccount) || "" == toElsAccount){
			alert("<i18n:I18n defaultValue='请选择对方ELS' key='i18n_label_additionCost_pleaseSelectToElsAccounts'/>!",2);
       		return;
       	}
		$("#querySaleOrderDialog").show();//mmgrid加载需指定宽高或知道父容器的宽高,父容器隐藏时无宽高,需先显示再加载或者指定一个宽高让mmgrid加载
		querySaleOrderGrid = $('#querySaleOrderTable').mmGrid({
			cols : querySaleOrderCols,
			url : "${pageContext.request.contextPath}/rest/SaleOrderService/querySaleOrderItemByHeadCondtion",
	        method : 'post',
	        autoLoad : false,
	        root : 'rows',
	        loadingText : "<i18n:I18n defaultValue='正在载入' key='i18n_common_mmgrid_loadingtext'/>",
	...
	//查询销售订单
	$("#querySaleOrderBtn").click(function(){
		var toElsAccount = $("#toElsAccount").val();
		...
		querySaleOrderGrid.load(param);
	});
	
	//从sap获取物料信息弹出窗确认按钮单击事件
	$("#querySaleOrderDialogOkBtn").click(function(){
		var selectRows = querySaleOrderGrid.selectedRows();
		if(selectRows.length == 0){
			//parent.elsDeskTop.tip.init({type : 'alert',displayType : 0,message : '请选择订单项！'});
			alert("<i18n:I18n defaultValue='请选择订单项!' key='i18n_delivery_pleaseSelectOrderItem'/>");
			return;
		}
		...
		$("#querySaleOrderDialog").hide();
		calcTotalAmount();//重算总额
	});
	
	//关闭窗口
	$("#querySaleOrderDialogClose").click(function(){
		$("#querySaleOrderDialog").hide();
	});
	
	//关闭窗口
	$("#querySaleOrderDialogCancelBtn").click(function(){
		$("#querySaleOrderDialog").hide();
	});
	//查询销售订单信息--------------end---------------
...
// 	   return "<input typpe='text' name='additionCostAmount' width='100%' class='additionCostAmountChange' value='"+val+"' />";
   }
   ...
   function itemRemarkRender(val,item,rowIndex){
	   ...
	   return "<input typpe='text' name='remark' width='100%' class='itemRemarkChange' value='"+val+"' />";
   }
