        ...
        <div class="box-wrap pos-relative bg-common">
            ...
            <div class="common-box-line">
				...
				<div class="dis-in-b input-and-tip"> 
					...
					<input type="text" id="additionCostDesc" name="additionCostDesc" width="100%"/>
				</div>
				<div class="dis-in-b input-and-tip"> 
...
<script>
	...
	var headCols = <base:tableColumn elsAccount="${elsAccount}" elsSubAccount="${elsSubAccount}" tableCode="saleAdditionCostManageHead_mt"/>;
	var headGrid = $("#headTable").mmGrid({
		url : "${pageContext.request.contextPath}/rest/SaleAdditionCostService/querySaleAdditionCostHead",
		method :  "POST",
		autoLoad : false,
		cols : headCols,
		height:<base:tableHeight tableCode="saleAdditionCostManageHead_mt"/>,
		root : "rows",
	...
	var itemCols = <base:tableColumn elsAccount="${elsAccount}" elsSubAccount="${elsSubAccount}" tableCode="saleAdditionCostManageItem_mt"/>;
	...
	debugger;
	var fileCols = <base:tableColumn elsAccount="${elsAccount}" elsSubAccount="${elsSubAccount}" tableCode="saleAdditionCostManageFile_mt"/>;
	...
	function init() {
	    ...
	    if("undefined"==typeof(additionCostNumber) || "null"==additionCostNumber){
			...
			costDateFrom = "";//请求页面时有传条件则忽略costDateFrom,costDateTo按请求条件查找
			costDateTo = "";
		}
	...
	$("#addBtn").click(function(){
	    parent.elsDeskTop.defineWin({
	        ...
	        windowsId : "saleAdditionCostAdd",
	        windowTitle : "<i18n:I18n defaultValue='新建附加费用单' key='i18n_windowTitle_additionCostAdd'/>",
	        iframSrc : "${pageContext.request.contextPath}/busreconmanage/sale/saleAdditionCostAdd.jsp",
	        showAdv : true,
			...
			if((additionCostStatus == "0" && sendStatus == "0") || additionCostStatus == "3"){
				if(person == currPerson && "add" == headRecord.additionCostType){
					parent.elsDeskTop.defineWin({
				        ...
				        iframSrc : "${pageContext.request.contextPath}/busreconmanage/sale/saleAdditionCostUpd.jsp?"
					...
					alert("<i18n:I18n defaultValue='非本单据制单人,禁止修改' key='i18n_alert_canotUpd_notSalePerson'/>",2);
				}
			} else {
				alert("<i18n:I18n defaultValue='非本方新建未发送单据,禁止修改' key='i18n_alert_canotUpd_hasSend'/>",2);
			}
		}
	});
		...
		if("undefied" != typeof(selectRows) && "[]" != selectRows && "" != selectRows){
			...
			if((additionCostStatus == "0" && sendStatus == "0") || additionCostStatus == "3"){
				if(person == currPerson && "add" == headRecord.additionCostType){
					...
					alert("<i18n:I18n defaultValue='非本单据制单人,禁止删除' key='i18n_alert_canotUpd_notSalePerson'/>",2);
				}
			} else {
				alert("<i18n:I18n defaultValue='非本方新建未发送单据,禁止删除' key='i18n_alert_canotUpd_hasSend'/>",2);
			}
		}
	});
	...
	$("#confirmBtn").click(function(){
		...
		if("undefied" != typeof(selectRows) && "[]" != selectRows && "" != selectRows){
			var headRecord = selectRows[0];
			if(headRecord.additionCostStatus == "2"){
	...
	$("#sendBtn").click(function(){
		var selectRows = headGrid.selectedRows();
		if("undefied" != typeof(selectRows) && "[]" != selectRows && "" != selectRows){
			var saleAdditionCostHead = selectRows[0];
			var saleAdditionCostItem = itemGrid.rows();
		   	var saleAdditionCostFile = fileGrid.rows();
			...
			alert("<i18n:I18n defaultValue='非本方单据，禁止发送' key='i18n_common_sendfail'/>",2);
			return;
		}
	});
	...
	$("#columnDefineBtn").click(function(){
		parent.elsDeskTop.defineWin({
		    ...
		    windowSubTitle : "",
		    iframSrc : "${pageContext.request.contextPath}/tablecolumn/subAccountColumnSetting.jsp?elsAccount=" + elsAccount + "&elsSubAccount="+elsSubAccount + "&tableCode=saleAdditionCostManageHead_mt&window=iframeApp_saleAdditionCostManage",
		    showAdv : true,
	...
	$("#itemDefineBtn").click(function(){
		parent.elsDeskTop.defineWin({
		    ...
		    windowTitle :"<i18n:I18n defaultValue='列自定义' key='i18n_common_button_columncustom'/>",
		    windowSubTitle : "",
		    iframSrc : "${pageContext.request.contextPath}/tablecolumn/subAccountColumnSetting.jsp?elsAccount=" + elsAccount + "&elsSubAccount="+elsSubAccount + "&tableCode=saleAdditionCostManageItem_mt&window=iframeApp_saleAdditionCostManage",
		    showAdv : true,
		    advArr : ["icon/test-car.png","icon/test-car2.png"],
		    windowMinWidth : 480,
		    windowMinHeight : 600,
