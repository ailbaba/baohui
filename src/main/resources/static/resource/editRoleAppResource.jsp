...
<script type="text/javascript">
	var instance,roleCode,queryUrl,sendUrl,appCode
	...
	queryData  = function () {
		queryUrl = "<%=basePath %>rest/RoleResourceService/getRoleAppResourceTree/" + roleCode + "/" + appCode;
	...
	saveData  = function(){
		parent.parent.elsDeskTop.tip.init({type: 'confirm',message:'<i18n:I18n key="i18n_common_confirm_save" defaultValue="是否确认保存？" />',closeEvent:function(){
			var s = '';
			sendUrl	= "<%=basePath %>rest/RoleResourceService/updateRoleAppResource";
			...
			$.ajax({
				...
				data : JSON.stringify({elsAccount:elsAccount,elsSubAccount:elsSubAccount,roleCode:roleCode,appCode:appCode,resourceCode:s}),
				dataType : "json",
	...
	function closeWin(){
		parent.elsDeskTop.closeCurWin('editRoleAppResource');
	}
