    ...
    <div class="pos-relative public-wrap">
        <div class="box-wrap pos-relative bg-common">
            ...
            <div class="common-box-line">
                ...
                <button id="differConfirmBtn" class="button-wrap"><i18n:I18n defaultValue="差异确认" key=""/> </button>
...
<script>
	...
	var enquiryHandle = function(val, item, index){
		return '<span class="delItemBtn" style="color:blue;cursor:pointer"><i18n:I18n key="i18n_button_delete" defaultValue="删除" /></span><span style="display: none;">'+JSON.stringify(item)+'</span>';
	};
	var itemCols = [
			...
			{ title:"采购方库存状态", name:"purchaseStockStatus", width:100, hidden:true, align:"center",renderer:renderItemResult},
...
// 			{ title:"物料行编号", name:"materialLineNumber", width:100, align:"center"},
	...
	$(".delItemBtn").live("click", function() {
        ...
        if(voucherStatus == "0"){
        	...
        	alert("<i18n:I18n defaultValue='已发送,禁止删除' key='i18n_alert_canotDel_hasSend'/>");
        }
	});
    ...
    $("#addBtn").click(function(){
	    parent.elsDeskTop.defineWin({
	        ...
	        iframSrc : "${pageContext.request.contextPath}/physicalinventory/purchase/purchasePhysicalInventoryManageAdd.jsp?addFlag=p",
	        showAdv : true,
	...
	$("#confirmBtn").click(function(){
		...
		if("undefied" != typeof(head) && "[]" != head && "" != head){
			...
			if(voucherStatus == "1"){
				$.ajax({
					url :"${pageContext.request.contextPath}/rest/PhysicalInventoryService/purchaseConfirmPhysicalInventory",
					type :"post",
					contentType : "application/json",
	...
	$("#sendBtn").click(function(){
		...
		if("undefied" == typeof(item) || "[]" == item || "" == item){
			alert("您所选择头信息没有行！",2);
			return;
		}
	    var jsonObj ={
		...
		var voucherStatus = headRecord.voucherStatus;
		var sendStatus = headRecord.sendStatus;
		if(voucherStatus == "0" && sendStatus=="0"){
			parent.elsDeskTop.tip.init({type: 'confirm',message:'<i18n:I18n key="i18n_common_are_you_sure_you_want_to_do_this"    defaultValue="您确定要执行此操作吗？" />',closeEvent:function(){
				$.ajax({
					url 		: "${pageContext.request.contextPath}/rest/PhysicalInventoryService/purchaseSendPhysicalInventory",
					type 		: "post",
					contentType : "application/json",
			...
			alert('<i18n:I18n key="i18n_label_Sent" defaultValue="已发送" />',2);
		}
	});
	
	$("#differConfirmBtn").click(function(){
		var head = headGrid.selectedRows();
		var item = itemGrid.rows();
		if(head.length<=0){
			alert("请选择",2);
		...
		if(confirmStatus==2){
			alert('<i18n:I18n key="i18n_saleorder_confirmed" defaultValue="已确认" />',2);
			return;
		}
		var voucherStatus = headRecord.voucherStatus;
		if(voucherStatus != "4" ){
