  ...
  <div class="box-wrap pos-relative bg-common">
    ...
    <div class="common-box-line">
	    <div>
	        ...
	        <base:auditConfig elsAccount="${elsAccount }" businessType="delivery">
 ...
 <div class="tab-wrap" id="sheet-project">
    ...
    <div class="common-box-line">
    	<div class="edit-box-wrap">
			...
			<button id="fillTime" class="button-wrap"><i18n:I18n key="i18n_label_fillcolumn" defaultValue="填充列" /></button>
		</div>
       <main class="grid-container">
            <div id="table-item-div" class="grid-100">
...
<script type="text/javascript">
	//列
	var cols_head = <base:tableColumn elsAccount="${elsAccount}" elsSubAccount="${elsSubAccount}" tableCode="deliveryHead_mt"/>;
...
// 		}
		selectedItems = "";
		var itemList = itemData.materialVoucherItemList;
		for (var i=0;i<itemList.length;i++){
			selectedItems += itemList[i].saleOrderNumber+"-"+itemList[i].orderItemNumber + ",";
		}
...
// 		$("#selectedItems").val(selectedItems);
		...
		var url;
		if (isTemp) {
			url = "<%=basePath%>rest/MaterialVoucherService/saveTempDelivery";
		} else {
			url = "<%=basePath%>rest/MaterialVoucherService/saveDelivery";
		}
...
// 		}
		parent.elsDeskTop.tip.init({type: 'confirm',message:'<i18n:I18n key="i18n_common_are_you_sure_you_want_to_do_this" defaultValue="您确定要执行此操作吗？" />',closeEvent:function(){
			...
			selectedItems = selectedItems.substring(0,selectedItems.length - 1);
			$("#selectedItems").val(selectedItems);
			var url = "<%=basePath%>rest/MaterialVoucherService/sendDelivery";
			var frm = $("#form");
			parent.elsDeskTop.showLoading();
			$.ajax({
				url :url,
...
/* 					if (data.statusCode != "200") {
	...
	$('#printBtn').click(function() {
		...
		var head =headGrid.row(0) ;
		var datas =  itemGrid.rows()||[];
		datas.forEach(function (d) {
			var rows = [];
			var mixData = $.extend(true, {},head, d);
			rows.push(mixData);
			d.rows = rows;
		});
		//存储打印需要的数据
...
function fillData(data){
	...
	if (data.materialVoucherHeaderVO.fbk1) {
		fbk1 = data.materialVoucherHeaderVO.fbk1;
	}
	if (data.materialVoucherHeaderVO.fbk2) {
		fbk2 = data.materialVoucherHeaderVO.fbk2;
	}
...
$("#columnDefineBtn").click(function(){
	parent.elsDeskTop.defineWin({
        ...
        windowSubTitle: "",
        'iframSrc':'<%=basePath%>tablecolumn/subAccountColumnSetting.jsp?elsAccount=' + elsAccount + "&elsSubAccount="+elsSubAccount + "&tableCode=editDelivery_mt" + "&window=iframeApp_editDelivery",
        windowMinWidth: 480,
        windowMinHeight: 600,
        'windowWidth':480,
        'windowHeight':600,
...
$("#headColumnDefineBtn").click(function(){
	parent.elsDeskTop.defineWin({
        ...
        'windowTitle':'<i18n:I18n key="i18n_common_button_columncustom" defaultValue="列自定义" />',
        windowSubTitle: "",
        'iframSrc':'<%=basePath%>tablecolumn/subAccountColumnSetting.jsp?elsAccount=' + elsAccount + "&elsSubAccount="+elsSubAccount + "&tableCode=deliveryHead_mt" + "&window=iframeApp_editDelivery",
        windowMinWidth: 480,
        windowMinHeight: 600,
        'windowWidth':480,
        'windowHeight':600,
...
function getDeliveryQuery(){
	...
	for(var i=0;i<allRows.length;i++){
		...
		var saleOrderNumber=allRows[i].saleOrderNumber;
		if(orderItemNumber!=""&&orderItemNumber!=null){
			...
			if(maxDelivery&&maxDelivery!="0"){
				itemNumberJson[saleOrderNumber+"_"+orderItemNumber]=parseFloat(maxDelivery)-parseFloat(deliveredQuantity);
			}else{
				var quantity=allRows[i].orderQuantity;
				itemNumberJson[saleOrderNumber+"_"+orderItemNumber]=parseFloat(quantity)-parseFloat(deliveredQuantity);
			}
		}
	}
}
...
function check(){
	...
	$('#table-item').find("tr").each(function(){
		...
		if(orderItemNumber!=""&&orderItemNumber!=null){
			var wesbz=$(this).find(".WESBS").val();
			var deliveryQuantity=itemNumberJson[saleOrderNumber+"_"+orderItemNumber];
// 			var saleOrderNumber=$(this).find("input[name='materialVoucherItemList[][saleOrderNumber]']").val();
...
//给可以填充的列添加选择框
function showCheck(){
	$("#table-item-div").find("th").each(function(){
		var className=$(this).attr("class").split(" ")[0];
		console.log(className);
		var addCheck="<input type='checkbox' class='mmg-column-check'>";
		var count=$("."+className).find("input[name^='materialVoucherItemList[]']").not(":hidden").length;
		if(count>0){
...
function fill(){
	...
	var columnSelected=$("#table-item-div").find("th input:checked");          
	...
	$("#table-item-div").find("th input:checked").each(function(){
