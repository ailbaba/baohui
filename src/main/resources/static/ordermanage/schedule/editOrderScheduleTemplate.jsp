...
var cols_step = [
                   ...
                   { title:'<i18n:I18n key="i18n_schedule_title_cycletype" defaultValue="周期类型" />', name:'cycleType' ,width:150, align:'center',renderer:function(val){
                  	 return "<span class='showText' style='display:none'>" + val + "</span>" + "<select name='stepList[][cycleType]'><option value='H'><i18n:I18n key='i18n_schedule_selector_hour' defaultValue='小时' /></option><option value='D'><i18n:I18n key='i18n_schedule_selector_day' defaultValue='天' /></option><option value='W'><i18n:I18n key='i18n_schedule_selector_week' defaultValue='周' /></option><option value='M'><i18n:I18n key='i18n_schedule_selector_month' defaultValue='月' /></option></select>";
                   }},
                  ...
                  { title:'<i18n:I18n key="i18n_schedule_title_quantity" defaultValue="基准数量" />', name:'quantity' ,width:180, align:'center' ,renderer:function(val){
                	    if (!val) {val = "0.00";}
...
$("#saveBtn").click(function(){
	...
	if(templateCode == null || templateCode == "null" || templateCode == ""){
		...
		url = "<%=basePath%>rest/OrderScheduleService/saveTemplate";
	}
	var frm = $("#form");
...
$("#exitBtn").click(function(){
	parent.elsDeskTop.closeCurWin('editOrderScheduleTemplate',closeCallback);
});
