...
!function($){
    ...
    MMGrid.prototype = {
        ...
        //增加一个setClass字段，用来改变th样式，可从外部初始化的时候传进来
        , _titleHtml: function(col, rowspan){
            var opts = this.opts;
            var titleHtml = [];
            var setBg=col.setClass?(" "+col.setClass + " "):'';
            if(!col.cols){
                titleHtml.push('<th class="'+setBg);
        ...
        , _initHead: function(){
			...
			//拖曳
            /*$head.find('th').draggable({
				addClasses : false,
				containment : '.mmGrid',
				cancel : '.mmg-colResize',
				handle : '.mmg-title',
				axis : 'x',
				cursor: 'move',
				cursorAt : {
				...
				helper : 'clone',
				revert : true,
				revertDurationType : 100,
				snap : true,
				snapTolerance : 30,
				snapMode : 'inner',
				drag : function (e, ui) {
					$head.helper = ui.helper;
					$head.dragElm = e.target;
					ui.helper.css({
						'border' : 0
					});
				},
			...
			$head.find('th').droppable({
				...
				drop : function (e, ui) {
					if ($head.helper && $head.dragElm) {
						$head.helper.hide();
						delete $head.helper;
						var thList = $head.find('th');
						var i = ~~(thList.index(e.target) > thList.index($head.dragElm));
						exchangeList = [$(e.target), $($head.dragElm)],
						a = exchangeList[1 - i],
						b = exchangeList[i],
						n = a.next(),
						p = b.prev();
            ...
            //列冻结功能
           /* if(parent.elsDeskTop && parent.elsDeskTop.contextMenu) {
            	//初始化表格列右键
        		var data=[
        			[{
        				text:"冻结至此列",
        		...
        		var subTh = $head;
            	parent.elsDeskTop.contextMenu($(document.body),data,"subth",10);
            }*/
        }
        ...
        , _initEvents: function(){
            ...
            $body.on('click','td',function(e){
              ...
              //当前列不允许点击触发事件
                ...
                //var colIndex = className.match(/col(\d+)$/)[1];
                //匹配首次出现col数字的字符
                var colIndex = className.match(/col(\d+)/)[1];
                var oInd = parseInt(colIndex);
                var pOpt =that.opts;
                //针对下拉框
                /*if($this.children().find('select') && $this.children().find('select').height() && pOpt.cols[oInd].clickDisable){
                    return;
                }*/
                //针对单元格里面的内容
                if(pOpt.cols[oInd] && pOpt.cols[oInd].clickDisable){
                    return;
                }
                //
        ...
        //替换渲染后的标签样式，用作自定义列
        ,_replaceStyle: function(str,text){
	    	if(str) {
	    		return str.toString().replace(/<\w+ (.*?)\/?>/g,function(b){
	  		      if(-1===b.indexOf('style')){return b.replace(/ /,' style=\''+text+'\' ')}
	  		      else{return b.replace(/style='/,'style=\''+text+';')}
	  		   })
	    	} else {
	    		return '';
	    	}
        ...
        , _rowHtml: function(item, rowIndex){
            ...
            if($.isPlainObject(item)){
                ...
                for(var colIndex=0; colIndex < leafCols.length; colIndex++){
                    ...
                    trHtml.push('<td style="position:relative;"  class="');
                    ...
                    trHtml.push('">');
                    if(col.renderer){
                    	var replaceTxt = 'text-align:'+col.align;
                    	var str = this._replaceStyle(col.renderer(item[col.name],item,rowIndex,col.name), replaceTxt);
        ...
        , _loadAjax: function(args){
            ...
            for(var i=0; i< this.opts.plugins.length; i++){
            	var obj ={};
            	if(args && args.page) {
            		obj.currentPage = args.page;
            	} 
            	if(args && args.limit) {
            		obj.pageSize = args.limit;
            	}
            	if($.isEmptyObject(obj)) {
                var plugin = this.opts.plugins[i];
                $.extend(pluginParams, plugin.params());
                    plugin.load(pluginParams);
            	} else {
                    var plugin = this.opts.plugins[i];
                    $.extend(pluginParams, plugin.params(obj));
                    plugin.load(pluginParams);
            	}
            }
        ...
        , addRows: function(item) {
            ...
            for(var i=0; i < item.length; i++){
            	strArray.push(this._rowHtml(item[i], this.rowsLength()));
            }
            var str = String.prototype.concat.apply("", strArray);
            $tbody.append(str);
            //绑定数据
            var $trs = $tbody.find("tr");
            var len = $trs.length;
            for(var i=0; i < item.length; i++){
            	$trs.eq(len-item.length+i).data("item",item[i]);
            }
        }
        //添加数据，第一个参数可以为数组
        ...
        //隐藏行，参数为索引
        , hideIndexRow: function(index) {
        	if(typeof(index)=='undefined') {
        		return
        	} else {
        		var that = this;
        		var $tbody = that.$body.find('tbody');
            	$tbody.find('tr').eq(index).hide();
        	}
        }
        //显示已隐藏的行,index有值显示index行,不传显示全部
        , showIndexRow: function(index) {
        	var that = this;
    		var $tbody = that.$body.find('tbody');
        	if(typeof(index)=='undefined') {
        		$tbody.find('tr').show();
