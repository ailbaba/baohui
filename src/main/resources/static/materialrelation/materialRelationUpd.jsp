...
<script type="text/javascript">
	...
	var id = '<%= request.getParameter("kId") %>';
	
	$(function() {
		 $.ajax({
				url :"../rest/MaterialRelationService/findMaterialRelation/"+elsAccount+"/"+id,
				type :"POST",
				contentType : "application/json",
				dataType : "json",
				success : function(data) {
					$("#id").val(data.id);
					$("#saleElsAccount").val(data.saleElsAccount);
					$("#purchaseElsAccount").val(data.purchaseElsAccount);
					$("#saleMaterial").val(data.saleMaterial);
					$("#saleMaterialDesc").val(data.saleMaterialDesc);
					$("#purchaseMaterial").val(data.purchaseMaterial);
					$("#purchaseMaterialDesc").val(data.purchaseMaterialDesc);
				},
				error : function(data) {
					showMsg("请求出错!");
				}
	...
	function materialRelationUpd(){
		...
		$.ajax({
			url :"../rest/MaterialRelationService/materialRelationUpd/"+elsAccount,
			type :"POST",
			contentType : "application/json",
...
<form id="materialRelationForm" action="" method="post">
	...
	<div style="text-align: center; padding: 5px">
			<input type="button" onclick="materialRelationUpd();"  value="确定"/>
